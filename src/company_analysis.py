...
def _request_with_retry(url: str, *, method: str = "get", max_retries: int = 1,
    ...
    None döner (bunlar tekrar denemekle genelde çözülmez).

    Reddit RSS gibi kaynaklar birkaç ardışık istekten sonra 429 döndürebiliyor
    (bkz. modül docstring'i — kırılgan kaynaklar). Sunucu `Retry-After` header'ı
    verirse ona uyulur, vermezse backoff_seconds kadar beklenir (üst sınır
    _RETRY_MAX_WAIT_SECONDS). Son deneme de başarısızsa None döner; çağıran
    taraf mevcut "sessiz boş liste" davranışını aynen sürdürür.
    """
...
def _reddit_rss(company: str, max_items: int = 8) -> list[dict]:
    ...
    Reddit birkaç ardışık istekten sonra 429 (çok istek) döndürebiliyor;
    bu yüzden _request_with_retry ile bir kez kısa beklemeli yeniden deneme yapılır.
    """
...
def _duckduckgo_search(query: str, max_items: int = 8) -> list[dict]:
    ...
    resp = _request_with_retry(url, method="post")
    if resp is None:
        return []
